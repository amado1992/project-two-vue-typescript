...
<AuthenticatedLayout>
    ...
    <form @submit.prevent="submit" style="text-align: center;">
      <h4> {{ $t('clients.import') }}</h4>
      <input type="file" @change="fileChanged" />
      <button type="submit" :disabled="!file">Importar</button>
      
    </form>
    <a href="/download/2" style="display:grid;justify-content: center;color: darkblue;margin:10px 0px">Descargar plantilla</a>
  </q-card>
        </div>
  </AuthenticatedLayout>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        this.$inertia.post('/import', formData).then( /* manejar la respuesta */ );
      }
    }
  };
  </script>